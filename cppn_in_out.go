...
func (i *CppnInOut) validate() {
	...
	for _, in := range i.Inputs {
		if inStrings(i.Outputs, in) {
			panic(fmt.Errorf("CppnInOut has both input and output named '%s'", in))
		}
	}

	// Neither inputs or outputs can be named "b", which is the name of the bias.
	if inStrings(i.Inputs, NODE_BIAS) {
		panic(fmt.Errorf("CppnInOut has input named same as the bias '%s'", NODE_BIAS))
	}
	if inStrings(i.Outputs, NODE_BIAS) {
		panic(fmt.Errorf("CppnInOut has output named same as the bias '%s'", NODE_BIAS))
	}
	...
	for _, in := range i.Inputs {
		...
		if err == nil {
			panic(fmt.Errorf("CppnInOut has input named as a number '%s'. Used for hidden nodes.", in))
		}
	}
	...
	for _, out := range i.Outputs {
		...
		// There is only a problem if we did NOT get an error. We successfully parsed the string to an integer which is bad.
		if err == nil {
			panic(fmt.Errorf("CppnInOut has output named as a number '%s'. Used for hidden nodes.", out))
		}
	}
}
